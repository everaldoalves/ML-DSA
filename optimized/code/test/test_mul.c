...
int main(void) {
  ...
  for(i = 0; i < NTESTS; ++i) {
    ...
    //poly_uniform(&b, seed, nonce++);
     // Prepara os arrays para processamento em lote
        int batch_size = 2; // Processaremos 'a' e 'b' juntos
        poly *a_batch[2] = { &a, &b };
        uint16_t nonce_batch[2] = { nonce++, nonce++ };

        // Chama a função otimizada poly_uniform_neon
        poly_uniform(a_batch, seed, nonce_batch, batch_size);
