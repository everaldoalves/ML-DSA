...
#include <arm_neon.h>
#include "fips202.h"
#include <arm_acle.h>  // Necessário para utilizar instruções intrínsecas SHA-3
...
static unsigned int keccak_squeeze(uint8_t *out, size_t outlen, uint64_t s[25], unsigned int pos, unsigned int r) {
  ...
  // Processar o output em blocos vetorizados
  while(outlen) {
    if (pos == r) {
      KeccakF1600_StatePermute(s);
      pos = 0;
    ...
    // Processar blocos de 8 bytes por vez usando NEON, quando possível
    while (pos + 8 <= r && outlen >= 8) {
      uint64x1_t data = vld1_u64(&s[pos / 8]); // Carregar um bloco de 8 bytes
      vst1_u8(out, vreinterpret_u8_u64(data)); // Escrever 8 bytes
      out += 8;
      outlen -= 8;
      pos += 8;
    }

    // Caso não seja possível processar mais 8 bytes, usa loop simples para os restantes
    for (i = pos; i < r && i < pos + outlen; i++) {
      *out++ = s[i / 8] >> 8 * (i % 8);
    }
